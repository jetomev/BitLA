...
> Quick fix: make sure you are on the `main` branch, which contains `CMakeLists.txt`, `src/`, and the rest of the app. In a terminal run `git status -sb` and confirm it shows `## main`.
...
1. **Confirm files exist locally**
   ...
   - If `git status -sb` shows a different branch name (e.g., `work`), switch to the right branch with `git switch main`. If `main` doesn’t exist locally, create it from the latest commit with `git switch -c main`.
